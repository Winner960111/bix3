...
import { CANDIDATE_SUBMISSION_HOLD } from "../../../ApiUrl";
...
export default function CandidateList({
  ...
  const fetchCandidateList = () => {
    axios
      .post(CANDIDATE_SUBMISSION_HOLD, recObject, {
        headers: { Authorization: users.token },
      })
      .then((resp) => {
        setCandidateListDetails(resp.data.data.hold_candidate_list);
  ...
  return (
    <div>
      <Card title={"Hold by me"} bordered={false} className="px-0 py-0">
        {SubmissionsTableMe()}
      </Card>
