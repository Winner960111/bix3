...
export default function CreateOpening(props) {
  ...
  useEffect(() => {
    getStatesList();
    if (location.state) getOpeningDetail();
  }, []);
  ...
  const setOpeningsValues = (values) => {
    ...
    formRef.current.setFieldsValue({
      ...
      currency: "USD",
      contact_name:
        values.contact_name.length > 0 ? values.contact_name.map(user => user._id) : [],
  ...
  const onFinish = (values) => {
    ...
    const newValue = {
      account_name: users.user._id,
      contact_name: values.contact_name,
      opening_title: values.opening_title,
      opening_id: location.state
        ? openingId
        : users.user.company_code + Math.floor(100000 + Math.random() * 900000),
  ...
  return (
    <div>
      <Form ref={formRef} onFinish={onFinish} layout="vertical">
            ...
            <Col span={8}>
              <FormItem
                ...
                rules={[
                  {
                    required: true,
                    message: "Please Enter opening's Account Name.",
                  },
                ]}
              >
                <Input disabled={true} placeholder="Account Name" />
              </FormItem>
            </Col>
            <Col span={8}>
              <FormItem
        ...
        <Card className="mb-6">
          <Row>
            <Button onClick={handleClose} type="primary" danger className="mr-5 mb-2">
              Cancel
            </Button>
            <Button type="primary" className="d-flex align-items-center" htmlType="submit">
              Save
