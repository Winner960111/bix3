...
function CandidateProfileDetail(props) {
  ...
  const updateInterviewStatus = (record, status, values) => {
    if (record !== undefined) {
      ...
      const submission_id = job_opening_details ? job_opening_details._id : "";
      const params = {
        date_of_interview: values.date_of_interview,
        duration: values.duration,
        interview_type: values.interview_type,
        time_of_interview: values.time_of_interview,
        submission_id: job_opening_details ? job_opening_details._id : "",
        ...
        bdm_id:
          job_opening_details && job_opening_details.bdm_id
            ...
            : "",
        candidate_id:
          job_opening_details && job_opening_details.candidate_id
            ? job_opening_details.candidate_id
            : "",
        company_id: job_opening_details ? job_opening_details.company_id : "",
        message: "Interview has been scheduled on ",
        status: status,
        recruiter_id:
          job_opening_details && job_opening_details.recruiter_id
            ? job_opening_details.recruiter_id
            : "",
      ...
      setmsgError("");
      axios
        .post(COMPANY_INTERVIEW_SCHEDULE, params, {
          headers: { Authorization: users.token },
        })
        .then((res) => {
          if (!res.data.error) {
      ...
      return (
        <Row gutter={24} className="mb-5">
          <Col span={24} className="mb-10">
            ...
            <br />
            <Text type="secondary">
              {employer.work_since_from_month &&months.find((month) => month.value == employer.work_since_from_month).label +
                "/" +
                employer.work_since_from_year}
              to
              {!employer.is_current_company
                ? employer.work_since_to_month && months.find((month) => month.value == employer.work_since_to_month).label +
                "/" +
            ...
            <br />
            <Text type="secondary">
              Available to join in {noticePeriod.find((notice) => notice.value == employer.notice_period)?.label}
            </Text>
            <br />
  ...
  const UsersView = () => {
    ...
    const submission_status = userProfileDetails.candidate_submission?.submission_status;
    ...
    const statusObject = CandidateSubmissionStatus.find((status) => {
      return submission_status ? submission_status.toUpperCase() === status.value.toUpperCase() : false;
    });
    ...
    //   location.state &&
    //     item.interviewschedules &&
    //     item.interviewschedules.length > 0
    //     ? item.interviewschedules.find((object) => {
    //       return openingId === object.opening_id;
    //     })
    //     : undefined;
                ...
                <Avatar
                  ...
                  icon={
                    userProfileDetails.profile_image ?
                      ...
                      /> : <Title className="mb-0 text-white"> {(userProfileDetails.first_name && userProfileDetails.first_name !== '') ? userProfileDetails.first_name.charAt(0).toUpperCase() : ''} </Title>
                  }
                >
                      ...
                      <Col span="12" className="p-1">
                        ...
                        <Text className="text-white">
                          {userProfileDetails.current_ctc
                            ? parseInt(userProfileDetails.current_ctc) > 1000 ? '$ ' + parseInt(userProfileDetails.current_ctc) / 1000 + ' K' : userProfileDetails.current_ctc
                            : "$ 0.0"}
                        </Text>
                      </Col>
                    </Row>
          ...
          <Col span={12}>
            <Card>
              ...
              <Row gutter={24} className="mb-5">
                ...
                <Col span={12}>
                  <Text>
                    {job_opening_details?.short_description}
                  </Text>
                </Col>
              </Row>
              <Row gutter={24} className="mb-5">
                ...
                <Avatar
                  ...
                  icon={
                    userProfileDetails.profile_image ?
                      ...
                      /> : <Title className="mb-0 text-white">{userProfileDetails.first_name ? userProfileDetails.first_name.charAt(0).toUpperCase() : ''}</Title>
                  }
                >
                  <Title className="mb-0 text-white">J</Title>
                </Avatar>
                ...
                <Select
                  ...
                  value={
                    location.state
                      ...
                      : userProfileDetails.job_opening_details && userProfileDetails.job_opening_details.length > 0
            ...
            <Card title="Career Profile">
              <Row gutter={24} className="mb-5">
                <Col span={12}>
                  ...
                  <Text type="secondary">
                    {userProfileDetails &&
                      userProfileDetails.job_category && userProfileDetails.job_category.length > 0
                  ...
                  <Text type="secondary">
                    {
                      userProfileDetails.desired_employment_type
                        ? (optionEmploymentType.find(item => item.value === userProfileDetails.desired_employment_type)).label
                        : ''
                    }
