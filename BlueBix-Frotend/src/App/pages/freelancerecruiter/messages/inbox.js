...
export default function Inbox(props) {
  ...
  const getMessages = () => {
    const param = {
      ...
      freelancer_recruiter_id:
        users.role === "freelancerecruiter" ? users.user._id : "",
      candidate_id: "",
    };
    axios
  ...
  const singleItem = (item, index) => {
    ...
    const datetime = new Date(updated_at).toLocaleString("en-IN", options);
          ...
          <Row gutter={24}>
            <Col span={24} className={"p-3"}>
              <ListItem className="d-block">
                <NavLink
                  to={{
                    pathname: `/${users.role}/messages/MailDetail`, // "/freelancerecruiter/messages/MailDetail",
                    state: { item: item },
                  }}
                >
                  <div
