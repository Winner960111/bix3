...
import { user_login } from "../_redux/authCrud";
...
function Login(props) {
  ...
  const formik = useFormik({
    ...
    onSubmit: (values, { setStatus, setSubmitting }) => {  
      enableLoading();
      setTimeout(() => {
        const loginApiCall = user_login(values.email, values.password);
        ...
        loginApiCall
          .then((res) => {
            ...
            if (!result.data.error) {
              ...
              props.setRole(user.profile[0]);
              props.setUser(user);
              props.setToken(accessToken);
              // props.history.push("/dashboard");
              if (user.profile[0] === 'candidate') {
